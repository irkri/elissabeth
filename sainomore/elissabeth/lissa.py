...
class LISSALevel(HookedModule):
    """One level of the learnable iterated sum signature in the arctic
    semiring
    """

    _config_class = LISSALevelConfig
...
class LISSA(HookedModule):
    "Learnable Iterated Sums Signature in the arctic semiring"

    _config_class = LISSAConfig
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.levels: list[LISSALevel] = nn.ModuleList()  # type: ignore
        if (levels := self.config("lengths")) is not None:
            for p in levels:
                kwargs["length_is"] = p
                self.levels.append(LISSALevel(self, **kwargs))
        else:
